...
test.describe('Test Mode', () => {
  ...
  test('should display test introduction and details', async ({ page }) => {
    ...
    await expect(page.getByText('Ujian ini mengandungi 30 soalan pilihan ganda')).toBeVisible()
  })
  ...
  test('should show performance messages based on score', async ({ page }) => {
    ...
    await expect(page.getByText(/Sempurna|Cemerlang|Bagus|Lulus|Cuba Lagi/)).toBeVisible()
  })
