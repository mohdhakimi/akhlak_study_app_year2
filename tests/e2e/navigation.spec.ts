...
test.describe('Main Menu Navigation', () => {
  ...
  test('should display main menu with all navigation cards', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Akhlak Tahun Dua KSRI' })).toBeVisible()
    await expect(page.getByText('Aplikasi Pembelajaran Interaktif')).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Mod Belajar' })).toBeVisible()
    ...
    await expect(page.getByRole('heading', { name: 'Papan Markah' })).toBeVisible()
  })

  test('should navigate to Study Mode when clicked', async ({ page }) => {
    await page.getByRole('heading', { name: 'Mod Belajar' }).click()
    await expect(page).toHaveURL('/study')
    await expect(page.getByRole('heading', { name: 'Pilih Topik' })).toBeVisible()
  })
  ...
  test('should navigate to Test Mode when clicked', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Ujian Komprehensif' })).toBeVisible()
  })
