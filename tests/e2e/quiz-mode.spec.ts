...
test.describe('Quiz Mode', () => {
  ...
  test('should display quiz categories', async ({ page }) => {
    ...
    await expect(
      page.getByText('Mengenali dan mempraktikkan akhlak yang terpuji')
    ).toBeVisible()
    await expect(
  ...
  test('should start quiz when category is selected', async ({ page }) => {
    ...
    await expect(
      page.getByText('Apakah akhlak terpuji yang pertama?')
    ).toBeVisible()
  })
  ...
  test('should show performance messages based on score', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Sempurna|Cemerlang|Bagus|Cuba Lagi/)
    ).toBeVisible()
  })
})
