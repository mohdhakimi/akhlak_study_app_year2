...
test.describe('Study Mode', () => {
  ...
  test('should display topic selection with available topics', async ({
    page,
  }) => {
    await page.getByText('Mod Belajar').click()
    ...
    await expect(
      page.getByText(
        'Mengenali dan mempraktikkan akhlak yang terpuji dalam kehidupan seharian'
      )
    ).toBeVisible()
    await expect(
      page.getByText('Cara menghormati dan berbuat baik kepada ibu bapa')
    ).toBeVisible()
  })
  ...
  test('should show completion message on last note', async ({ page }) => {
    ...
    await expect(
      page.getByText('Syabas! Anda telah selesai mempelajari topik ini.')
    ).toBeVisible()
  })

  test('should format content with lists and bullet points', async ({
    page,
  }) => {
    await page.getByText('Mod Belajar').click()
    await page.getByText('Akhlak Terpuji').click()
    ...
    await expect(
      page.getByText('Jujur dalam perkataan dan perbuatan')
    ).toBeVisible()
    ...
    await expect(
      page.getByText('Membuatkan kita disukai oleh Allah SWT')
    ).toBeVisible()
  })
