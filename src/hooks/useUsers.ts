...
import { getUsers, saveUser, deleteUser, getCurrentUser, setCurrentUser, isValidUser } from '../utils/localStorage'
...
export function useUsers() {
  ...
  // Add a new user
  const addUser = useCallback((name: string): User | null => {
    if (!name.trim()) {
    ...
    const existingUser = users.find(u => u.name.toLowerCase() === name.toLowerCase().trim())
    if (existingUser) {
