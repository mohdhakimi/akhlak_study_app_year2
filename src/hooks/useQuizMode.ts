...
export function useQuizMode(): UseQuizModeReturn {
  ...
  const startQuiz = useCallback((selectedCategory: QuizCategory) => {
    ...
    const selectedQuestions = randomSelect(
      selectedCategory.questions,
      maxQuestions
    )
  ...
  const isComplete =
    quizState.currentQuestionIndex >= totalQuestions - 1 && quizState.isAnswered
  ...
  const currentCorrectAnswer =
    correctAnswers[quizState.currentQuestionIndex] || 0
  const currentShuffledOptions =
    shuffledOptions[quizState.currentQuestionIndex] || []

  const score = quizState.selectedAnswers.reduce(
