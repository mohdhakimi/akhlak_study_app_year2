...
const renderWithRouter = (component: React.ReactElement) => {
  return render(
    <BrowserRouter>
      <BilingualProvider>{component}</BilingualProvider>
    </BrowserRouter>
  )
}
...
describe('MainMenu', () => {
  ...
  it('displays welcome message with user name when user is logged in', () => {
    ...
    expect(
      screen.getByText('سلامت داتڠ، Ahmad! | Selamat Datang, Ahmad!')
    ).toBeInTheDocument()
  })
  ...
  it('shows correct button text based on user state', () => {
    ...
    renderWithRouter(<MainMenu />)
    expect(
      screen.getByRole('button', { name: /Pengguna semasa: Ahmad/ })
    ).toBeInTheDocument()
    ...
    expect(
      screen.getAllByRole('button', { name: /Pilih Pengguna/ })
    ).toHaveLength(2)
  })
