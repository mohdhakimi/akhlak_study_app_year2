...
const mockTopics: StudyTopic[] = [
  ...
  },
  {
    ...
    notes: [
      { id: 'note-3', title: 'Note 3', content: 'Content 3', order: 1 }
    ]
  }
]
...
describe('TopicSelector', () => {
  ...
  it('renders topic selector with all topics', () => {
    ...
    expect(screen.getByText('Pilih topik yang ingin anda pelajari')).toBeInTheDocument()
  ...
  it('shows empty state when no topics', () => {
    ...
    expect(screen.getByText('Maaf, tiada topik pembelajaran yang tersedia pada masa ini.')).toBeInTheDocument()
  })

  it('has responsive grid layout', () => {
    render(<TopicSelector {...defaultProps} />)
    
    const gridContainer = screen.getByText('Akhlak Terpuji').closest('div')?.parentElement
