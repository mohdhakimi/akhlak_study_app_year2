...
import contentData from '../../data/content-structure.json'
  ...
  describe('Content Quality', () => {
    it('should have age-appropriate content for 8-year-olds', () => {
      ...
      contentData.topics.forEach((topic) => {
        expect(topic.name.length).toBeLessThan(50) // Short, simple titles
        expect(topic.description.length).toBeLessThan(100) // Concise descriptions
        
        topic.notes.forEach((note) => {
          expect(note.title.length).toBeLessThan(60) // Short, simple titles
          expect(note.content.length).toBeLessThan(500) // Not too long for 8-year-olds
        })
      })
    })
    ...
    it('should have appropriate question difficulty', () => {
      contentData.quizCategories.forEach((category) => {
        category.questions.forEach((question) => {
          ...
          expect(question.explanation.length).toBeLessThan(200)
        })
      })
    })
